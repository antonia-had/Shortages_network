...
def build_url(app_data: dict, page_index: int) -> str:
    ...
    adminNo = get_app_data(app_data, 'ADMIN_NO')
    ...
    # Base URL
    url = 'https://dwr.state.co.us/Rest/GET/api/v2/analysisservices/callanalysisbywdid/?'
    # Append other parts
    if adminNo != '':
        url = "{}adminNo={}".format(url, adminNo)
    if wdid != '':
    ...
    if start_date != '':
        start_date = '%2F'.join(start_date.split('/'))
        url = "{}&startDate={}".format(url, start_date)
    if end_date != '':
        end_date = '%2F'.join(end_date.split('/'))
