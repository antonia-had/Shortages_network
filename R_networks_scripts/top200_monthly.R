...
m_list <- list.files(path='./network_csv_files/priorityWdid/monthly/')
#m <- '2000-01.csv'
...
for (m in m_list) {
  ...
  data <- read.csv(paste('./network_csv_files/priorityWdid/monthly/', m, sep = ""))
  ...
  #ggsave(p, filename = paste('./figures/monthly/', substring(m, 1, 7), '.png', sep=""), width = 10, height = 8)
...
df1 <- data.frame(month=substr(m_list, 1, 7), comps=t(comps))
p <- ggplot(data=df1, aes(x=month, y=comps)) +
  geom_bar(stat="identity", fill = 'skyblue') +
  geom_text(aes(label=comps), vjust=-0.3, size=3.5) +
  theme_classic() + 
  ggtitle("Number of network components each month") +
  labs(y="Number of components", x = "Month") +
...
df2 <- data.frame(month=substr(m_list,1,7), users=sr_users)
q <- ggplot(data=df2, aes(x=month, y=users)) +
  geom_bar(stat="identity", fill = 'cornflowerblue') +
  geom_text(aes(label=users), vjust=-0.3, size=3.5) +
  theme_classic() +
  ggtitle("Number of top 200 senior users each month") +
  labs(y="Number of users", x = "Month") +
  # scale_x_continuous(breaks = seq(2000, 2019, by = 1)) +
  theme(axis.text.x = element_text(size=10, angle=90))
q
